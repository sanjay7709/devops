...
resource "azurerm_app_service" "sanwebapp" {
  ...
  connection_string {
    name  = "Database"
    type  = "SQLServer"
    value = "Server=tcp:azurerm_sqldb.fully_qualified_domain_name Database=azurerm_sql_database.sqldb12.name;User ID=azurerm_sql_server.sqldb.administrator_login;Password=azurerm_sql_server.sqldb.administrator_password;Trusted_connection = false;Encrypt=True;Integrated Security=SSPI"
  }
}
...
resource "azurerm_sql_database" "sqldb12" {
